        ...
        <div class="col-12 col-sm-auto text-center">
            <div class="btn-group btn-group-sm w-100 d-flex flex-wrap flex-sm-nowrap" role="group">
                ...
                <form action="{{ route('documents.approve', $document) }}" method="POST" class="d-inline">
                    @csrf
                    @method('PATCH')
                    <button type="submit" class="btn @if($document->is_approved) btn-warning @else btn-success @endif d-flex align-items-center justify-content-center">
                        @if($document->is_approved)
                            <span class="d-none d-sm-inline">Отменить</span>
                            <span class="d-sm-none fas fa-times"></span>
                        @else
                            <span class="d-none d-sm-inline">Одобрить</span>
                            <span class="d-sm-none fas fa-check"></span>
                        @endif
                    </button>
                </form>
